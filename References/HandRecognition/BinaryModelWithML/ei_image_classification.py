...
port = pyb.UART(3, 9600, timeout_char = 1000)
port.init(9600, bits = 8, parity = None, stop = 1, timeout_char = 1000)
...
while(True):
    ...
    for obj in net.classify(img, min_scale=1.0, scale_mul=0.8, x_overlap=0.5, y_overlap=0.5):
        ...
        img.draw_string(0, 0, predictions_list[current_prediction_index][0], color = (255, 0, 0), scale = 2,
                        mono_space = False, char_rotation = 0, char_hmirror = False, char_vflip = False,
                        string_rotation = 0, string_hmirror = False, string_vflip = False)

        for i in range(len(predictions_list)):
            print("%s = %f" % (predictions_list[i][0], predictions_list[i][1]))
        if current_prediction_index < 4:
            if current_prediction_index == 0:
                ...
                blue_led.on()
                port.write('c')
            elif current_prediction_index == 2:
                red_led.off()
                green_led.on()
                blue_led.off()
                port.write('o')
            elif current_prediction_index == 3:
                red_led.off()
                green_led.off()
                blue_led.on()
                port.write('2')
